...
public class AlgoFHUQIMinerPSet {
...
//    private Map<Integer, Map<QitemCustom, List<QitemCustom>>> mapProjectedTrans;
    private Map<Integer, Map<QitemCustom, Integer>> mapProjectedTransUtility;
...
//        mapProjectedTrans = new HashMap<>();
        mapProjectedTransUtility = new HashMap<>();
...
//                List<QitemCustom> projectedTransaction = revisedTransaction.subList(i,revisedTransaction.size());
//                mapProjectedTrans.get(tid).put(q, projectedTransaction);

                mapProjectedTransUtility.computeIfAbsent(tid, k -> new HashMap<>());
                mapProjectedTransUtility.get(tid).put(q, remainingUtility);
    ...
    private void miner(QitemCustom[] prefix, int prefixLength, UtilityListCustom prefixUL,
        ...
        for (int i = 0; i < promisingQItems.size(); i++) {
            ...
            for (int j = i + 1; j < promisingQItems.size(); j++) {
                ...
                if (mapTWUF != null) {
                    Integer sum = mapTWUF.get(nextQitem);
                    if (sum == null)
                        continue;
                    sumTWU += sum;
                } else {
                    ...
                    for (int ii = currentQitem.getQuantityMin(); ii <= currentQitem.getQuantityMax(); ii++) {
                        ...
                        for (int tid : pSet) {
                            Integer sum = mapProjectedTransUtility.get(tid).get(exactQitem);
                            if (sum == null)
                                continue;
                            sumTWU += sum;
                        }
