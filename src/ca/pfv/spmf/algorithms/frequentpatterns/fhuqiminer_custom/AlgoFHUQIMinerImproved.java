...
public class AlgoFHUQIMinerImproved {
        ...
        for (int i = 0; i < promisingQItems.size(); i++) {
            ...
            if (currentQitem.isRange()) {
                for (int ii = currentQitem.getQuantityMin(); ii <= currentQitem.getQuantityMax(); ii++) {
                    t2[ii - currentQitem.getQuantityMin()] = promisingQItems.indexOf(new QitemCustom(currentQitem.getItem(), ii));
                }
            }
            ...
            for (int j = i + 1; j < promisingQItems.size(); j++) {
                ...
                for (int ii = currentQitem.getQuantityMin(); ii <= currentQitem.getQuantityMax(); ii++) {
                    ...
                    Integer sum = mapFMAP.get(exactQitem).get(nextQitem);
                    if (sum == null)
                        continue;
                    sumTWU += sum;
                }
